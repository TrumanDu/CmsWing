...
module.exports = class extends think.cmswing.center {
  ...
  async addcartAction() {
    ...
    for (const val of arr) {
      ...
      if (think.isEmpty(goods.suk)) {
        ...
        const suk = JSON.parse(goods.suk);
        const arr_ = val.type.split(',');
        const getpr = getsuk(suk.data, arr_);
        // console.log(getpr);
        if (suk.is_pic == 1) {
  ...
  async createorderAction() {
    ...
    let goodsids;
    const goodslist = JSON.parse(data.goodslist);
    const goodsarr = [];
    ...
    const isgoods = await this.model('cart').where({id: ['IN', goodsarr]}).select();
