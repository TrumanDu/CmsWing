...
module.exports = class extends think.cmswing.center {
  ...
  async selectaddrAction() {
    const get = this.get();
    const data = await this.model('address').where({user_id: this.user.uid}).order('is_default DESC,id DESC').select();
  ...
  async addaddrAction() {
    ...
    if (data.is_default == 1) {
      const find = await this.model('address').where({user_id: this.user.uid, is_default: 1}).select();
    ...
    if (res) {
      ...
      if (this.isMobile) {
        ...
        const addrlist = await this.model('address').where({user_id: this.user.uid}).order('is_default DESC,id DESC').select();
  ...
  async addrisdefaultAction() {
    ...
    const find = await this.model('address').where({user_id: this.user.uid}).order('is_default ASC').select();
    for (const val of find) {
      if (val.id == id) {
        val.is_default = 1;
      } else {
  ...
  async getaddrAction() {
    const id = this.get('id');
    const addr = await this.model('address').where({user_id: this.user.uid}).find(id);
  ...
  async deladdrAction() {
    ...
    const res = await this.model('address').where({user_id: this.user.uid, id: id}).delete();
    if (res) {
      const addrlist = await this.model('address').where({user_id: this.user.uid}).order('is_default DESC').select();
      for (const val of addrlist) {
        val.province = await this.model('area').where({id: val.province}).getField('name', true);
        val.city = await this.model('area').where({id: val.city}).getField('name', true);
        val.county = await this.model('area').where({id: val.county}).getField('name', true);
  ...
  async editaddrmodalAction() {
    ...
    if (!think.isEmpty(id)) {
      // 获取地址信息
      const address = await this.model('address').where({user_id: this.user.uid}).find(id);
