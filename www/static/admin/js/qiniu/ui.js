...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var viewProgess = $('<a class="btn btn-default">查看分块上传进度</a>');
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    var showImg = $('<img src="/static/admin/js/qiniu/loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', '/static/admin/js/qiniu/default.png');
        ...
        $(img).on('load', function() {
            ...
            function initImg(url, key, height) {
                ...
                modalBody.find('img').attr('src', '/static/admin/js/qiniu/loading.gif');
